...
CREATE POLICY "agent_qa_read_policy_v4"
  ON agent_qa
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "agent_qa_insert_policy_v4"
  ON agent_qa
  FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "agent_qa_update_policy_v4"
  ON agent_qa
  FOR UPDATE
  TO authenticated
  USING (true)
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_agent_qa_agent_id_v2 ON agent_qa(agent_id);
CREATE INDEX IF NOT EXISTS idx_agent_qa_status_v2 ON agent_qa(status);
